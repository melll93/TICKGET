 ...
 * @Component
 * 
 * @Log4j2 public class ChatHandler extends TextWebSocketHandler {
 * 
 * private static List<WebSocketSession> list = new ArrayList<>();
 * 
 * @Override protected void handleTextMessage(WebSocketSession session,
 * TextMessage message) throws Exception { String payload =
 * message.getPayload(); log.info("payload : " + payload);
 * 
 * for (WebSocketSession sess : list) { sess.sendMessage(message); } }
 * 
 ...
 * 
 * log.info(session + " 클라이언트 접속 해제"); list.remove(session); }
 * 
 * }
 */